...
class FeatureFusion(nn.Module):
    def __init__(self, channels):
        ...
        self.Attentionxq = nn.Sequential(
            ChannelAttention(1024+512+1024),
            nn.Conv2d(1024+512+1024, 1024+512+1024, kernel_size=1, bias=True),
            nn.PReLU()
        )
        self.Attentionx = nn.Sequential(
            ChannelAttention(1024+512),
            nn.Conv2d(1024+512, 1024+512+1024, kernel_size=1, bias=True),
            nn.PReLU()
        )
        self.Attentionq = nn.Sequential(
            ChannelAttention(1024),
            nn.Conv2d(1024, 1024+512+1024, kernel_size=1, bias=True),
            nn.PReLU()
        )
...
class UNet_Pytorch_DeepSup(torch.nn.Module):
    ...
    def __init__(self, n_input_channels=3, n_classes=7, n_filt=64, batchnorm=True, dropout=False, upsample="bilinear"):
        ...
        ##################################################################################################
        # # 前：
        # self.conv_1 = nn.Conv2d(in_channels=9,out_channels=9,kernel_size=1,stride=1)
        # self.conv_2 = nn.Conv2d(in_channels=9,out_channels=3,kernel_size=1,stride=1)
        
        # # 后：
        # self.conv_3 = nn.Conv2d(in_channels=256, out_channels=512, kernel_size=1, stride=2, padding=0)
        # self.conv_4 = nn.Conv2d(in_channels=512, out_channels=1024, kernel_size=1, stride=2, padding=0)
        ...
        # self.after_2_1=deconv2d(1024,1024,kernel_size=2,stride=2)
        # self.after_2_2=conv2d(1024,512)
        # self.after_2_3=conv2d(512,512)

        # self.after_3_1=deconv2d(512,512,kernel_size=2,stride=2)
        # self.after_3_2=conv2d(512,256)
        # self.after_3_3=conv2d(256,256)

        # self.after_4_1=deconv2d(256,256,kernel_size=2,stride=2)
        # self.after_4_2=conv2d(256,128)
        # self.after_4_3=conv2d(128,128)

        # self.fusion_module =  FeatureFusion(1024+512+1024)
        
        model_weight_path = "/home/crq/TractSeg/tractseg/models/1mobile_sam.pt"
    ...
    def forward(self, inpt):
        ...
        # image_embedding_2 = self.after_2_1(image_embedding)
        # image_embedding_2 = self.after_2_2(image_embedding_2)
        # image_embedding_2 = self.after_2_3(image_embedding_2)

        # image_embedding_3 = self.after_3_1(image_embedding_2)
        # image_embedding_3 = self.after_3_2(image_embedding_3)
        # image_embedding_3 = self.after_3_3(image_embedding_3)

        # image_embedding_4 = self.after_4_1(image_embedding_3)
        # image_embedding_4 = self.after_4_2(image_embedding_4)
        # image_embedding_4 = self.after_4_3(image_embedding_4)
...
# net = UNet_Pytorch_DeepSup(n_input_channels=9).to(device)

# summary(net, input_size=(9, 144, 144))
...
model = UNet_Pytorch_DeepSup(n_input_channels=9)
# device = torch.device("cuda:0") 
