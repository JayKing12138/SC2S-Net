...
class BaseModel:
    def __init__(self, Config, inference=False):
        ...
        if self.Config.LOSS_FUNCTION == "soft_sample_dice":
            ...
            # loss_function = getattr(importlib.import_module("tractseg.libs.Loss_function"), 'DC_and_Focal_loss')
            # self.criterion = loss_function(soft_dice_kwargs={} ,focal_kwargs={})
    ...
    def train(self, X, y, weight_factor=None):
        ...
        # outputs = self.net(X).detach().clone()  # (bs, classes, x, y)
        angle_err = None
        ...
        if weight_factor is not None:
            ...
            if self.Config.EXPERIMENT_TYPE == "peak_regression":
                ...
                loss = nn.BCEWithLogitsLoss(weight=weights)(outputs[4], y)
        else:
            if self.Config.LOSS_FUNCTION == "soft_sample_dice" or self.Config.LOSS_FUNCTION == "soft_batch_dice":
                # loss = self.criterion(F.sigmoid(outputs), y.detach_())
                ...
                # loss = self.criterion(outputs, y.detach_())
                # loss_1 = self.criterion(outputs[0], y)
                # loss_2 = self.criterion(outputs[1], y)
                # loss_3 = self.criterion(outputs[2], y)
                # loss_4 = self.criterion(outputs[3], y)
                # loss_5 = self.criterion(outputs[4], y)
        ...
        if self.Config.EXPERIMENT_TYPE == "peak_regression":
            ...
            #f1 = pytorch_utils.f1_score_macro(y.detach(), F.sigmoid(outputs[4]).detach(), per_class=True,
    ...
    def predict(self, X):
        ...
        if self.Config.EXPERIMENT_TYPE == "peak_regression" or self.Config.EXPERIMENT_TYPE == "dm_regression":
            ...
            # probs = F.sigmoid(outputs[4]).detach().cpu().numpy()
