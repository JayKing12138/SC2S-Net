...
class UNet_Pytorch_DeepSup_Double_Encoder2(torch.nn.Module):
    def __init__(self, n_input_channels=3, n_classes=7, n_filt=64, batchnorm=False, dropout=False, upsample="bilinear"):
        super(UNet_Pytorch_DeepSup_Double_Encoder2, self).__init__()
        ...
        self.pool_1_new = nn.MaxPool2d((2, 2))

        self.contr_2_1_new = conv2d(n_filt, n_filt * 2)
        self.contr_2_2_new = conv2d(n_filt * 2, n_filt * 2)
        self.pool_2_new = nn.MaxPool2d((2, 2))

        self.contr_3_1_new = conv2d(n_filt * 2, n_filt * 4)
        self.contr_3_2_new = conv2d(n_filt * 4, n_filt * 4)
        self.pool_3_new = nn.MaxPool2d((2, 2))

        self.contr_4_1_new = conv2d(n_filt * 4, n_filt * 8)
        self.contr_4_2_new = conv2d(n_filt * 8, n_filt * 8)
        ...
        self.expand_1_1 = conv2d(n_filt * 8 + n_filt * 16 + n_filt * 8, n_filt * 8)
        ...
        self.expand_2_1 = conv2d(n_filt * 4 *2 + n_filt * 8, n_filt * 4, stride=1)
        ...
        self.expand_3_1 = conv2d(n_filt * 2*2 + n_filt * 4, n_filt * 2, stride=1)
    ...
    def forward(self, inpt):
        ...
        pool_1_new = self.pool_1_new(contr_1_2)

        contr_2_1_new = self.contr_2_1_new(pool_1_new)
        contr_2_2_new = self.contr_2_2_new(contr_2_1_new)
        pool_2_new = self.pool_2_new(contr_2_2_new)

        contr_3_1_new = self.contr_3_1_new(pool_2_new)
        contr_3_2_new = self.contr_3_2_new(contr_3_1_new)
        pool_3_new = self.pool_3_new(contr_3_2_new)

        contr_4_1_new = self.contr_4_1_new(pool_3_new)
        contr_4_2_new = self.contr_4_2_new(contr_4_1_new)
        ##################################################
        ...
        concat1 = torch.cat([deconv_1, contr_4_2, contr_4_2_new], 1)
        expand_1_1 = self.expand_1_1(concat1)
        expand_1_2 = self.expand_1_2(expand_1_1)
        deconv_2 = self.deconv_2(expand_1_2)

        concat2 = torch.cat([deconv_2, contr_3_2, contr_3_2_new], 1)
        ...
        concat3 = torch.cat([deconv_3, contr_2_2, contr_2_2_new], 1)
